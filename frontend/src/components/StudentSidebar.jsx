...
const navItems = [
  { label: "My Exams", path: "/student/dashboard" },
  { label: "Start Exam", path: "/student/start" },
...
`}
      >
        <div className="h-28 flex items-center justify-between px-4 border-b border-[#3B82F6]/20 dark:border-[#3B82F6]/40 rounded-t-xl dark:rounded-t-2xl">
          <div className="flex flex-col items-start gap-1 flex-1">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-300">
              Signed in as <span className="font-semibold">{user?.username || user?.name || "Student"}</span>
            </div>
          </div>
        ...
        <nav className="mt-4 px-2 flex-1 overflow-y-auto">
          <p className="px-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-4">
            Student Panel
          </p>
      ...
      <div className="flex-1">
        ...
        <main className="p-4 md:p-8 lg:p-12 bg-white dark:bg-[#1D1A17] min-h-screen">{children}</main>
      </div>
    </div>
  );
}
